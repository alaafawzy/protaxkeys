...
import { Container, Grid, Stack, Step, StepConnector, StepLabel, Stepper, Typography, stepConnectorClasses } from '@mui/material';
...
const QontoConnector = styled(StepConnector)(({ theme }) => ({
  [`&.${stepConnectorClasses.alternativeLabel}`]: { top: 5, left: 'calc(-10% + 16px)', right: 'calc(50% + 16px)' },
  [`&.${stepConnectorClasses.active}`]: { [`& .${stepConnectorClasses.line}`]: { borderColor: '#784af4' } },
  [`&.${stepConnectorClasses.completed}`]: { [`& .${stepConnectorClasses.line}`]: { borderColor: '#784af4', minHeight: "2rem" } },
  [`& .${stepConnectorClasses.line}`]: { borderColor: '#eaeaf0', margin: "-1.4rem 0.6rem", borderTopWidth: 4, borderRadius: 1, minHeight: "2rem" },
}));
        ...
        <Stack>
          <Stepper alternativeLabel activeStep={1} connector={<QontoConnector />} orientation="vertical">
            {steps.map((item, index) => (
              <Step key={index} sx={{ display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-around" }}>
                <StepLabel StepIconComponent={QontoStepIcon} sx={{ display: "flex", flexDirection: "row !important", minWidth: "100%" }}
                ...
                {item.download ? (
                  <Grid sx={{ border: "2px solid #D0D5DD", borderRadius: "8px", background: '#F9FAFB', color: "#131F89", display: "flex", padding: "0.5rem", width: "60%", justifyContent: "space-around", cursor: "pointer", fontFamily: "Tajawal", fontWeight: 500, fontSize: "14px", marginTop: "0.5rem" }}>
                    <Grid><svg xmlns="http://www.w3.org/2000/svg" width="19" height="18" viewBox="0 0 19 18" fill="none"><path d="M6.16699 13.1667L9.50033 16.5M9.50033 16.5L12.8337 13.1667M9.50033 16.5V9M16.167 12.9524C17.1849 12.1117 17.8337 10.8399 17.8337 9.41667C17.8337 6.88536 15.7816 4.83333 13.2503 4.83333C13.0682 4.83333 12.8979 4.73833 12.8054 4.58145C11.7187 2.73736 9.71235 1.5 7.41699 1.5C3.96521 1.5 1.16699 4.29822 1.16699 7.75C1.16699 9.47175 1.8632 11.0309 2.98945 12.1613" stroke="#131F89" strokeWidth="1.66667" strokeLinecap="round" strokeLinejoin="round" /></svg></Grid>
                    تنزيل
                  </Grid>
                ) : null}
              </Step>
